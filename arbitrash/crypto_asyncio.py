...
class crypto_asyncio:
	name = 'crypto'
	urlDepth = 'https://api.crypto.com/v2/public/get-book?depth=5&instrument_name='
	def __init__(self,timeout:int):
		self.timeout = timeout
	def geturl(self,token):
		return str('https://crypto.com/exchange/trade/spot/' + token + '_USDT')
		...
		async with aiohttp.ClientSession() as session:
			try:
				...
				async with session.get(url=url,timeout=self.timeout) as response:
					...
					price_ask = 0
					price_bid = 0
					json_response = json_response['result']['data'][0]
