...
class ftx_asyncio:
	name = 'ftx'
	#urlDepth = 'https://ftx.com/api/markets/BTC_USDT/orderbook?depth=10'
	def __init__(self,timeout:int):
		self.timeout = timeout
	def geturl(self,token):
		return str('https://ftx.com/trade/' + token + '/USDT')


	async def depth(self, token:str):
		#url = self.urlDepth + token + '_USDT'
		url = 'https://ftx.com/api/markets/'+token+'_USDT/orderbook?depth=10'
