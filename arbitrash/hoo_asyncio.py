...
class hoo_asyncio:
	name = 'hoo'
	urlDepth = 'https://api.hoolgd.com/open/v1/depth/market?symbol='
	def __init__(self,timeout:int):
		self.timeout = timeout
	def geturl(self,token):
		return str('https://hoo.com/spot/' + token  +'-USDT')
			...
			try:
				...
				async with session.get(url=url,timeout=self.timeout) as response:
					...
					for ask in json_response['asks'][:5]:
						sum_asks = sum_asks + float(ask['price']) * float(ask['quantity'])
						price_ask = price_ask + float(ask['price'])
					for bid in json_response['bids'][:5]:
						sum_bids = sum_bids +float(bid['price']) * float(bid['quantity'])
						price_bid = price_bid +float(bid['price'])
