...
class kraken_asyncio:
	name = 'kraken'
	urlDepth = 'https://api.kraken.com/0/public/Depth?count=5&pair='
	def __init__(self,timeout:int):
		self.timeout = timeout
	def geturl(self,token):
		return str('https://www.kraken.com/en-us/prices/' + token  )


	async def depth(self, token:str):
		url = self.urlDepth + token + 'USD'
		...
		async with aiohttp.ClientSession() as session:
			try:
				...
				async with session.get(url=url,timeout=self.timeout) as response:
					...
					price_ask = 0
					price_bid = 0
					json_response = json_response['result'][token+'USD']
