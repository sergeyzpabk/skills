...
class poloniex_asyncio:
	name = 'poloniex'
	urlDepth = 'https://poloniex.com/public?command=returnOrderBook&depth=5&currencyPair='
	def __init__(self,timeout:int):
		self.timeout = timeout
	def geturl(self,token):
		return str('https://poloniex.com/exchange/' + 'USDT_' + token  )


	async def depth(self, token:str):
		url = self.urlDepth + 'USDT_' + token
